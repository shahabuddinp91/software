...
$validation = true;
    ...
    $execute_discount = $conn->query($select_discount);
        ...
        if($validation){
            //echo $bill.' ' .$discount.' '.$total_bill.' '.$payment.' '.$due;
            $select_query="SELECT * FROM `diagonostic_patient_bill` WHERE regi_id='$get_patinet_id'";
            //echo $select_query;
            //die();
            ...
            if($count==0){
                $inser_sql="INSERT INTO `diagonostic_patient_bill`(`regi_id`, `bill`, `discount`, `total_bill`, `payment`, `due`, `date`,`month`,`year`) VALUES ('$get_patinet_id','$bill','$discount','$total_bill','$payment','$due','$today','$month','$year')";
            //    echo $inser_sql;
            //    die();
            ...
            if($count == 1){
                ...
                $update_sql="UPDATE `diagonostic_patient_bill` SET `bill`='$bill',`discount`='$discount',`total_bill`='$total_bill',`payment`='$paymentnew',`due`='$new_due' WHERE regi_id='$get_patinet_id'";
