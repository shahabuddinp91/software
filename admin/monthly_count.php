...
<?php include './left_side_admin.php'; ?>
    ...
    font-size: 28px;
    padding-bottom: 7px;
    padding-left: 183px;">Advance search for monthly payment count</h2>
                                <?php 
                                $message = '';
                                $count = '';
                                include '../connection/db.php';
                                    if(isset($_POST['total'])){
                                        ...
                                        if($total_month == 1 or $total_year == 1){
                                            $message = 'Select month & year !';
                                        }  else {
                                            $select_value = "SELECT * FROM `daily_payment` WHERE month = '$total_month' and year = '$total_year'";
                                            $execute_value = $conn->query($select_value);
                                            $count = mysqli_num_rows($execute_value);
                                            if($count > 0){
                                                $message = 'Search complete for month : '.$total_month.' & year : '.$total_year;
                                                $select_all = "SELECT sum(rec_total), sum(rec_pay),sum(rec_due),sum(dia_total),sum(dia_pay),"
                                                        . "sum(dia_due),sum(phar_total),sum(phar_pay),sum(phar_due),sum(acc_total),sum(acc_pay),sum(acc_due),sum(total),sum(total_pay),sum(total_due) FROM `daily_payment` WHERE month = '$total_month' and year = '$total_year'";
                                                ...
                                                $message = 'Data not found for month : '.$total_month.' & year : '.$total_year;
                                            }
                                        }
                                    }
                                ?>
    ...
    margin-top: 34px;">
                                        ...
                                        <div class="col-md-3">
                                            <?php
                                                if($count > 0) {  
                                                $select_debit = "SELECT sum(Total) FROM `debit` WHERE month = '$total_month' and year = '$total_year'";
                                                $exe_debit = $conn->query($select_debit);
                                                while ($row = $exe_debit->fetch_assoc()){
                                                    $debit  = $row['sum(Total)'];
                                                }
                                                $select_salary = "SELECT sum(payment) FROM `salary` WHERE month = '$total_month' and year = '$total_year'";
                                                $exe_salary = $conn->query($select_salary);
                                                while ($row = $exe_salary->fetch_assoc()){
                                                    $salary = $row['sum(payment)'];
                                                }
                                                
                                                $ref_in_select = "SELECT sum(payment) FROM `in_reference` WHERE month = '$total_month' and year = '$total_year'";
                                                $exe_in = $conn->query($ref_in_select);
                                                while ($row = $exe_in->fetch_assoc()){
                                                    $in_ref = $row['sum(payment)'];
                                                }
                                                
                                                $ref_out_select = "SELECT sum(payment) FROM `out_reference` WHERE month = '$total_month' and year = '$total_year'";
