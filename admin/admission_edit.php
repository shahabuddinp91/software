...
//                            $get_id='';
                            ...
                            $select_admission = "SELECT * FROM `patient_admission_system` WHERE reg_id = '$get_id'";
                            $execute_admission = $conn->query($select_admission);
                            while ($row = $execute_admission->fetch_assoc()){
                            ...
                            $mess = '';
                                if(isset($_POST['update'])){
                                    ...
                                    $update_sql = "UPDATE `patient_entry_form` SET `patient_name`='$patient_name',`mobile`='$patient_mobile',`reference`='$ref_name',`ref_mobile`='$ref_mobile' WHERE id = '$id'";
                                    $execute_update = $conn->query($update_sql);
                                    if($execute_update){
                                        $up_admi = "UPDATE `patient_admission_system` SET `doctor_name`='$Doctor_Name' WHERE  reg_id = '$id'";
                                        $ex_up = $conn->query($up_admi);
                                ...
                                <form class="" style="margin-bottom: 200px;" method="post" action="">
                                    <p style="color: red;text-align: center;text-transform: uppercase;"><?php echo $mess;?></p>
                                        ...
                                        <div class="col-md-3 form-group">
                                            ...
                                            <input style="text-align: center;color: #16A05D;" type="text" name="Doctor_Name" value="<?php echo $doctor_name;?>">
                                        </div>
